 ...
 *
 * Identical prompts are used on both mobile (ExecuTorch) and web
 * (cloud-backed). Only the inference engine changes — prompt behavior
...
- category: one of [food_dining, groceries, transportation, utilities, entertainment, healthcare, education, shopping, travel, business, rent_housing, subscriptions, personal_care, donations, taxes_fees, other]
...
- spendingTrends: array of { period, amount, changePercent, direction } showing spending patterns over time
- recurringMerchants: array of { merchant, totalSpent, transactionCount, isRecurring, averageAmount } for frequently visited merchants
- budgetWarnings: array of { category, currentSpend, projectedSpend, severity, message } for categories with concerning spending
...
- unusualExpenses: array of string descriptions of unusual spending patterns
...
Categories: food_dining, groceries, transportation, utilities, entertainment, healthcare, education, shopping, travel, business, rent_housing, subscriptions, personal_care, donations, taxes_fees, other

Return ONLY a JSON object with a single "category" field. No markdown, no explanation.
Example: {"category": "food_dining"}
...
Generate the following:
- anomalies: array of { type, description, severity, relatedTransaction } where type is one of [amount_spike, unusual_merchant, duplicate, time_anomaly, category_mismatch]
