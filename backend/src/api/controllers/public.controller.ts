  ...
  async (_req: Request, res: Response) => {
      ...
      User.aggregate([
        {
          $match: {
            role: ROLES.OWNER,
            "companyInfo.companyType": { $exists: true, $ne: null },
          },
        },
    ...
    const successRate =
      totalVerifications > 0
        ? Math.round((verifiedCount / totalVerifications) * 1000) / 10
        : 0;
    const data = {
      companies,
      branches,
      ...
      companiesByType: companiesByTypeAgg.map((c) => ({
        type: c._id,
        count: c.count,
      })),
      verificationsByProvider: verificationsByProviderAgg.map((v) => ({
        provider: v._id,
        count: v.count,
      })),
      trustedCompanies: trustedCompanies
        .filter((u: any) => u.companyInfo?.companyName)
        .map((u: any) => ({
          companyName: u.companyInfo.companyName,
          companyType: u.companyInfo.companyType,
          website: u.companyInfo.website,
          country: u.companyInfo.country,
          region: u.companyInfo.region,
          city: u.companyInfo.city,
          address: u.companyInfo.address,
        })),
      platformLaunchYear: earliestOwner?.createdAt
        ? new Date(earliestOwner.createdAt).getFullYear()
