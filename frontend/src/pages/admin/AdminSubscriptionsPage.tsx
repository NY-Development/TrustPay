  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold font-headline-md text-gray-900 dark:text-white">Subscriptions Catalog</h1>
          <p className="text-xs text-gray-500">Track active billing contract records, verify manual bank deposits, and adjust payment statuses.</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#131b2e] border border-[#c2c6d9]/30 rounded-xl overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-xs text-gray-400">No subscriptions found match query keys.</div>
        ) : (
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-900 border-b border-[#c2c6d9]/20 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  ...
                  <th className="px-6 py-4">TXN Connection</th>
                </tr>
              </thead>
                ...
                {subscriptions.map((s: any) => (
                  <tr key={s._id} className="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4">
                      {s.userId ? (
                        <>
                          <div className="font-semibold text-gray-900 dark:text-white">{s.userId.name}</div>
                          <span className="text-[10px] text-gray-400 font-mono">{s.userId.email}</span>
                        </>
                      ) : (
                        <span className="text-gray-405 font-bold italic">User registry removed</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-xs uppercase font-bold text-red-650 dark:text-red-400">{s.plan}</div>
                      <span className="text-[10px] text-gray-550 dark:text-gray-400">
                      ...
                      <button
                        ...
                        className={`text-xs font-bold px-2 py-1 rounded cursor-pointer transition-colors ${
                          s.fullyPaid
                            ...
                            : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-905/20 dark:text-yellow-450'
                        }`}
                      >
